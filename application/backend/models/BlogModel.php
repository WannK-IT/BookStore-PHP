...
class BlogModel extends Model
{
	...
	public function listItems($arrParams)
	{
		...
		//search
		$query[] = (!empty($arrParams['search_value'])) ? "AND `title` LIKE '%" . trim($arrParams['search_value']) . "%'" : '';
	...
	{
		$query[] 	= "SELECT `title`, `content`, `status`, `picture`, `ordering`";
		$query[] 	= "FROM `{$this->table}`";
		$query[] 	= "WHERE `id` = '" . $id . "'";
		$query		= implode(" ", $query);
	...
	public function formHandle($arrParams, $paramsUrl, $option)
	{
		...
		if ($option == 'add') {
			require_once EXTEND_PATH . 'Upload.php';
			$upload = new Upload();
			$newPicture = $upload->uploadFile($arrParams['picture'], 'blog');

			// $arrParams['created'] set default ở phpmyadmin
			$arrParams['posted_by'] = $fullName['fullname'];
			$arrParams['posted_date'] = date('Y-m-d H:i:s');
			...
			if ($arrParams['picture']['name'] == null) {
				...
				$oldImg = self::singleItem($paramsUrl['id']);
				$oldImg = $oldImg['picture'];
				$upload->deleteFile(UPLOAD_PATH . 'blog', $oldImg);
				$newPicture = $upload->uploadFile($arrParams['picture'], 'blog');
				$arrParams['picture'] = $newPicture;
			}
	...
	public function deleteItem($arrParams)
	{
		...
		foreach ($arrImg as $value) {
			$upload->deleteFile(UPLOAD_PATH . 'blog', $value);
		}
