...
$(document).ready(function () {
    ...
    // change ordering slider
    $('.chkOrderingSlider').change(function (e) {
        if ($(this).val() < 1) {
            e.preventDefault();
            toastMsg('warning', 'Giá trị Ordering thấp nhất là 1');
        } else {
            ...
            let id = $(this).data('id');
            let value = $(this).val();
            let link = 'index.php?module=backend&controller=slider&action=changeOrdering&newValue=' + value + '&id=' + id;
            $.get(link, function (data) {
            });
        }
    })
