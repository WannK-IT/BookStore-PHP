...
class BookModel extends Model
{
	...
	{
		$query[] 	= "SELECT `id`, `name`, `price`, `special`, `sale_off`, `picture`,	`created`, `created_by`, `modified`, `modified_by`,	`status`, `ordering`, `category_id`";
	...
	{
		$query[] 	= "SELECT `name`, `description`, `price`, `special`, `sale_off`, `picture`, `status`, `ordering`, `category_id`";
		$query[] 	= "FROM `{$this->table}`";
		$query[] 	= "WHERE `id` = '" . $id . "'";
		$query		= implode(" ", $query);
	...
	public function changeSpecial($arrParams)
	{
		$id = $arrParams['id'];
		$special = ($arrParams['special'] == 'no') ? 'yes' : 'no';
		$query = "UPDATE `{$this->table}` SET `special` = '$special' WHERE `id` = '$id'";
		$this->query($query);

		return [$id, $special, URL::createLink($arrParams['module'], $arrParams['controller'], 'changeSpecial', ['special' => $special, 'id' => $id])];
	}
	...
	public function deleteItem($arrParams)
	{
		...
		foreach ($arrImg as $value) {
			$upload->deleteFile(UPLOAD_PATH . 'book', $value);
		}
	...
	public function formHandle($arrParams, $paramsUrl, $option)
	{
		...
		if ($option == 'add') {
			require_once EXTEND_PATH . 'Upload.php';
			$upload = new Upload();
			$newPicture = $upload->uploadFile($arrParams['picture'], 'book');
			...
			if ($arrParams['picture']['name'] == null) {
				...
				$upload->deleteFile(UPLOAD_PATH . 'book', $oldImg);
				$newPicture = $upload->uploadFile($arrParams['picture'], 'book');
				$arrParams['picture'] = $newPicture;
			}
	...
	public function changeGroupCategory($idElement, $idGroupSelected)
	{
		$query = "UPDATE `{$this->table}` SET `category_id` = '{$idGroupSelected}' WHERE `id` = '{$idElement}'";
		$this->query($query);
